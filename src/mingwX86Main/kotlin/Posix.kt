...
actual fun memset(__s: CValuesRef<*>?, __c: Int, __n: ULong): CPointer<out CPointed>? {
    return platform.posix.memset(__s, __c, __n.toUInt())
}
...
actual fun MemScope.sockaddrIn(sin_family: UShort, sin_port: UShort): sockaddr_in {
    ...
    sockaddr.sin_port = htons(sin_port.convert()).convert()
    return sockaddr
}
...
actual fun set_non_blocking(__fd: Int): Int {
    memScoped {
        ...
        on.value = 1u
        return platform.posix.ioctlsocket(__fd.toUInt(), platform.posix.FIONBIO.toInt(), on.ptr.reinterpret())
    }
}
